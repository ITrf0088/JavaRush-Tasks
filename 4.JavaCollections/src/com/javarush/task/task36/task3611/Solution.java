...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        for (int i = 0; i < humanRelationships.length; i++) {
            if((humanRelationships[index].length > i) && humanRelationships[index][i]){
                set.add(i);
                set.addAll(getAllFriendsAndPotentialFriends(i,deep));
            }
            if((humanRelationships[i].length > index) && humanRelationships[i][index]){
                set.add(i);
                set.addAll(getAllFriendsAndPotentialFriends(i,deep));
            }
        }
    ...
    private static boolean[][] generateRelationships() {
        return new boolean[][]{
                ...
                {true, true, false, true, true, false, true, false, true, false, true},                                        //4
